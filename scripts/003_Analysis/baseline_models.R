...
#  0.5216184 (so I got 0.658 after running this)
...
models <- c(model_dat_filt_cc, trad_impute, mice_imputed)
...
for(model in models){
  ...
  pred_class_B <- list()
  for(i in seq_along(model_form)) {
    X_train <- data.matrix(model[, sleep_preds][train_ind, ])
    y_train <- data.matrix(model[, outcome_vars[i]][train_ind, ])
    X_test <- data.matrix(model[, sleep_preds][test_ind, ])
    y_test <- data.matrix(model[, outcome_vars[i]][test_ind, ]) 
